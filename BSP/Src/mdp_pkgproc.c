...
uint8_t  ExamPkgFmt             (const MDP_PkgFmt *PkgFmt);
...
{
	ioDevice->PkgProcessor.FormatEnable = FormatEnable;
	ioDevice->PkgProcessor.DataDst = Container;
	ioDevice->PkgProcessor.RawDataSize = RawDataSize;
	ioDevice->PkgProcessor.PkgProcesscb = CallbackFunc;

	if(FormatEnable==1)
	{
		/* Default Parse Protocol:
		 * 1.Enable package head, head bytes are 0xC8 and 0xFF;
...
void io_SetPkgParseFmt(MDP_io *ioDevice, const MDP_PkgFmt *PkgFmt)
{
	...
	{
		printf("[error  ] set package parse error:package format incorrect.");
	}
...
uint8_t _io_PackageProcess(MDP_io *ioDevice)
{
	...
	{
		printf("[error ] io device parsing package error:package format illegal.");
		return 0;
	}
 ...
 *     4.PkgFmt:pointer of data package format struct--'MDP_PkgFmt'.
 * @retval: none
 * @author: Wang Geng Jie
 *****************************************************************************************************/
void io_SendDataPackage(MDP_io *ioDevice, void *pData, uint32_t Length, const MDP_PkgFmt *PkgFmt)
{
	...
	if(!ExamPkgFmt(PkgFmt))
	{
		printf("[error] send data package error:data package format is not correct, please check the package format.");
		return ;
	}

	if(PkgFmt->HeadEnable)
	{
		io_SendData(ioDevice, (void*)PkgFmt->Head, PkgFmt->HeadLength);
	}
	...
	if(!PkgFmt->CheckValEnable&&PkgFmt->TailEnable)
	{
		io_SendData(ioDevice, (void*)PkgFmt->Tail, PkgFmt->TailLength);
	}
...
uint8_t _io_GetRawData(MDP_io *ioDevice)
{
	...
	{
		printf("[error] io device find data package error:both raw data size and tail aren't specified.");
		return 0;
	}
	...
	if(ioDevice->PkgProcessor.RawDataSize==0 && ioDevice->PkgProcessor.PackageFormat->TailEnable==0)
	{
		printf("find data package error:Both raw data size and package tail aren't specified.");
		return 0;
	}
